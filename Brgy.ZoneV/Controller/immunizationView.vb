Imports MySql.Data.MySqlClient

Public Class immunizationView
    Dim connString As String = "datasource=localhost;port=3306;username=root;password=;database=bzvhcims;"

    Private birthingId As Integer

    Public Sub New(babysName As String, birthingId As Integer)
        ' This is the constructor of the form, which will be called when creating a new instance.
        InitializeComponent()

        Me.birthingId = birthingId ' Use Me keyword to refer to the class-level variable
        txtPatient.Text = babysName

        ' Call the method to load data based on the weighingId
        LoadData()
    End Sub
    ' Create a method to load data into the DataGridView
    Private Sub LoadData()
        Dim query As String = "SELECT birthing_id, date_of_vaccination, vaccine_type,
            ROW_NUMBER() OVER (PARTITION BY birthing_id ORDER BY immunization_save_record_id) AS immune_id   
            FROM immunizationsaverecord WHERE birthing_id = @birthingId
            ORDER BY immunization_save_record_id ASC"

        Using conn As New MySqlConnection(connString)
            Try
                conn.Open()

                Using cmd As New MySqlCommand(query, conn)
                    cmd.Parameters.AddWithValue("@birthingId", Me.birthingId) ' Use Me keyword to refer to the class-level variable

                    Dim adapter As New MySqlDataAdapter(cmd)
                    Dim dataTable As New DataTable()

                    adapter.Fill(dataTable)

                    ' Set the DataSource to the DefaultView of the DataTable
                    immuneDataGridView.DataSource = dataTable.DefaultView
                End Using
            Catch ex As Exception
                MessageBox.Show($"Error loading data: {ex.Message}")
            End Try
        End Using
    End Sub

    Private Sub immuneDt_ValueChanged(sender As Object, e As EventArgs) Handles immuneDt.ValueChanged
        Dim selectedDate As DateTime = immuneDt.Value

        ' Extract month and year from the selected date
        Dim selectedMonth As Integer = selectedDate.Month
        Dim selectedYear As Integer = selectedDate.Year

        ' Construct the SQL query with parameters for month and year
        Dim query As String = "SELECT birthing_id, date_of_vaccination, vaccine_type, ROW_NUMBER() OVER (ORDER BY immunization_save_record_id) AS immune_id " &
                             "FROM immunizationsaverecord " &
                             "WHERE MONTH(date_of_vaccination) = @selectedMonth AND YEAR(date_of_vaccination) = @selectedYear AND birthing_id = @birth"

        Using conn As New MySqlConnection(connString)
            Try
                conn.Open()

                Using cmd As New MySqlCommand(query, conn)
                    ' Add parameters for the selected month and year
                    cmd.Parameters.AddWithValue("@selectedMonth", selectedMonth)
                    cmd.Parameters.AddWithValue("@selectedYear", selectedYear)
                    cmd.Parameters.AddWithValue("@birth", birthingId)

                    Dim adapter As New MySqlDataAdapter(cmd)
                    Dim dataTable As New DataTable()

                    adapter.Fill(dataTable)

                    ' Set the DataSource to the DefaultView of the DataTable
                    immuneDataGridView.DataSource = dataTable.DefaultView
                End Using
            Catch ex As Exception
                MessageBox.Show($"Error loading data: {ex.Message}")
            End Try
        End Using
    End Sub
End Class